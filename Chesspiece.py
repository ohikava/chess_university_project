...
from math import pi, sqrt, copysign, cos, sin
...
class Bishop(Chesspiece):
    ...
    def move_to(self, position: complex, new_position: complex, move=1) -> (bool, str, tuple):
        position_difference = new_position - position
        ...
        if phase(position_difference) * 180 / pi in {45, 135, -45, -135}:
            ...
            while i != new_position:
                i += complex(copysign(1, cos(phase(position_difference))), copysign(1, sin(phase(position_difference))))
                trajectory.append(i)
            return True, '', trajectory
